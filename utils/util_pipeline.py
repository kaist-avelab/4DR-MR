...
from models.skeletons import build_skeleton, build_skeleton_KD
from models.prediction import build_prediction_skeleton
import datasets
...
def build_network_KD(p_pline):
    return build_skeleton_KD(p_pline.cfg_teacher)

def build_network_multi(cfg):
    return build_skeleton_KD(cfg)
...
def build_prediction(p_pline):
    cfg_prediction = p_pline.cfg.get('PREDICTION', None)
    if cfg_prediction is None:
        return None
    else:
        return build_prediction_skeleton(cfg_prediction)

def build_dataset(p_pline, split='train'):
    return datasets.__all__[p_pline.cfg.DATASET.NAME](cfg = p_pline.cfg, split=split)

def build_dataset_KD(cfg, split='train'):
    return datasets.__all__[cfg.DATASET.NAME](cfg, split=split)
