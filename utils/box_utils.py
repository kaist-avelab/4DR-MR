...
def boxes_giou(box_a, box_b):
    ...
    enclosing_width = enclosing_x_max - enclosing_x_min
    enclosing_height = enclosing_y_max - enclosing_y_min
...
def convert_3d_boxes_to_bev_corners(boxes_3d):
        """
        Args:
            boxes_3d: (7) shaped tensor, each row [x, y, z, dx, dy, dz, theta]

        Returns:
            corners: (8) shaped tuple, each row [x1, y1, x2, y2, x3, y3, x4, y4]
        """
        
        x, y, _, dx, dy, _, theta = boxes_3d
        ...
        corner_x3 = x + dx/2 * cos_theta + dy/2 * sin_theta
        corner_y3 = y + dx/2 * sin_theta - dy/2 * cos_theta
        corner_x4 = x - dx/2 * cos_theta + dy/2 * sin_theta
        corner_y4 = y - dx/2 * sin_theta - dy/2 * cos_theta

        corners = (corner_x1, corner_y1, corner_x2, corner_y2,corner_x3, corner_y3, corner_x4, corner_y4)

        return corners



def cal_iou_3d(box3d1:torch.Tensor, box3d2:torch.Tensor):
    """calculated 3d iou. assume the 3d bounding boxes are only rotated around z axis

    Args:
        box3d1 (torch.Tensor): (7),  (x,y,z,l,w,h,theta)
        box3d2 (torch.Tensor): (7),  (x,y,z,l,w,h,theta)
    """
    box1 = convert_3d_boxes_to_bev_corners(box3d1)
    box2 = convert_3d_boxes_to_bev_corners(box3d2)

    poly1 = geom.Polygon([(box1[i], box1[i + 1]) for i in range(0, len(box1), 2)])
    poly2 = geom.Polygon([(box2[i], box2[i + 1]) for i in range(0, len(box2), 2)])

    intersection = poly1.intersection(poly2).area
    union = poly1.area + poly2.area - intersection

    iou_2d = intersection / union if union != 0 else 0
    ...
    intersection_3d = iou_2d * union * z_overlap
...
def box3d_lidar_to_box2d(boxes3d):
    bev_boxes = []
    for box_3d in boxes3d:
        x, y, z, dx, dy, dz, heading = box_3d

        # 2D BEV 박스의 x1, y1, x2, y2 좌표를 계산합니다.
        x1 = x - dx / 2.0
        y1 = y - dy / 2.0
        x2 = x + dx / 2.0
        y2 = y + dy / 2.0

        bev_boxes.append([x1, y1, x2, y2])

    return np.array(bev_boxes)

def box3d_lidar_to_box2d_one(boxes3d):
    bev_boxes = []

    x, y, z, dx, dy, dz, heading = boxes3d
...
def center_to_corner2d(center, dim):
    corners_norm = torch.tensor([[-0.5, -0.5], [-0.5, 0.5], [0.5, 0.5], [0.5, -0.5]]).type_as(center)  # (4, 2)

    corners = dim.view([1, 2]) * corners_norm.view([4, 2])  # (4, 2)
    corners = corners + center.view(1, 2)
    return corners
...
def bbox3d_overlaps_diou_dif(pred_boxes, gt_boxes):
    """
    https://github.com/agent-sgs/PillarNet/blob/master/det3d/core/utils/center_utils.py
    Args:
        pred_boxes (M, 7): 
    ...
    dious = torch.zeros((num_pred, num_gt))
    ...
    for i in range(num_pred):
        for j in range(num_gt):
            ...
            qcorners = center_to_corner2d(pred_boxes[i, :2], pred_boxes[i, 3:5])  # (4, 2)
            gcorners = center_to_corner2d(gt_boxes[j, :2], gt_boxes[j, 3:5])  # (4, 2)   
            ...
            # calculate area
            volume_pred_boxes = pred_boxes[i, 3] * pred_boxes[i, 4] * pred_boxes[i, 5]
            volume_gt_boxes = gt_boxes[j, 3] * gt_boxes[j, 4] * gt_boxes[j, 5]

            inter_h = torch.minimum(pred_boxes[i, 2] + 0.5 * pred_boxes[i, 5], gt_boxes[j, 2] + 0.5 * gt_boxes[j, 5]) - \
                    torch.maximum(pred_boxes[i, 2] - 0.5 * pred_boxes[i, 5], gt_boxes[j, 2] - 0.5 * gt_boxes[j, 5])
            inter_h = torch.clamp(inter_h, min=0)
            ...
            # boxes_iou3d_gpu(pred_boxes, gt_boxes)
            inter_diag = torch.pow(gt_boxes[j, 0:3] - pred_boxes[i, 0:3], 2).sum(-1)

            outer_h = torch.maximum(gt_boxes[j, 2] + 0.5 * gt_boxes[j, 5], pred_boxes[i, 2] + 0.5 * pred_boxes[i, 5]) - \
                    torch.minimum(gt_boxes[j, 2] - 0.5 * gt_boxes[j, 5], pred_boxes[i, 2] - 0.5 * pred_boxes[i, 5])
            outer_h = torch.clamp(outer_h, min=0)
            outer = torch.clamp((out_max_xy - out_min_xy), min=0)
            outer_diag = outer[0] ** 2 + outer[1] ** 2 + outer_h ** 2

            dious[i, j] = volume_inter / volume_union - inter_diag / outer_diag
            dious[i, j] = torch.clamp(dious[i, j], min=-1.0, max=1.0)
