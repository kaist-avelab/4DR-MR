...
from models.skeletons.radar_KRKD import radar_KRKD
...
class PipelineDetection_vI_T_R2R():
    def __init__(self, path_cfg_1=None, mode='train', path_cfg_2=None):
        '''
        ...
        *   'test'  denotes mode for inference
        '''
        cfg_2 = cfg.copy()
        self.cfg = cfg_from_yaml_file(path_cfg_1, cfg)
        self.cfg2 = cfg_from_yaml_file(path_cfg_2, cfg_2)
        self.cfg2 = AttrDict(self.cfg2)
        self.mode = mode
        ...
        self.dataset_train_2 = build_dataset_KD(self.cfg2, split='train') if self.mode == 'train' else None
        ...
        self.model = radar_KRKD(model_cfg= self.cfg.KRKD_CFG)
        self.model.model_radar1 = build_network(self).cuda()
        self.model.model_radar2 = build_network_multi(self.cfg2).cuda()

        self.optimizer = build_optimizer(self, self.model)
        self.scheduler = build_scheduler(self, self.optimizer)
        self.epoch_start = 0

        # Logging
        if self.cfg.GENERAL.LOGGING.IS_LOGGING:
            self.set_logging(path_cfg_1)
    ...
    def set_logging(self, path_cfg, is_print_where=True):
        ...
        if self.is_save_model:
            os.makedirs(os.path.join(self.path_log, 'models_1'))
            os.makedirs(os.path.join(self.path_log, 'models_2'))
            os.makedirs(os.path.join(self.path_log, 'utils_1'))
            os.makedirs(os.path.join(self.path_log, 'utils_2'))
    ...
    def train_network(self, is_shuffle=True):
        ...
        common_sampler = CommonSampler(self.dataset_train, shuffle=True)

        data_loader_train = torch.utils.data.DataLoader(self.dataset_train, \
            batch_size = self.cfg.OPTIMIZER.BATCH_SIZE, sampler= common_sampler, \
            collate_fn = self.dataset_train.collate_fn,
            num_workers = self.cfg.OPTIMIZER.NUM_WORKERS, drop_last = True)
        
        data_loader_train_2 = torch.utils.data.DataLoader(self.dataset_train_2, \
            batch_size = self.cfg.OPTIMIZER.BATCH_SIZE, sampler= common_sampler, \
            collate_fn = self.dataset_train_2.collate_fn,
            num_workers = self.cfg.OPTIMIZER.NUM_WORKERS, drop_last = True)

        # self.load_dict_model(self.model.model_radar1, self.cfg['MODEL'].PRETRAINED)
        self.load_dict_model(self.model.model_radar1, self.cfg['MODEL'].PRETRAINED)
        self.load_dict_model(self.model.model_radar2, self.cfg2['MODEL'].PRETRAINED)
        # self.model.model_radar.load_params_from_file()
        ...
        for epoch in range(epoch_start, epoch_end):
            ...
            self.model.model_radar1.train()
            self.model.model_radar2.train()
            self.model.model_radar1.training = True
            self.model.model_radar2.training = True
            
            # self.model.model_radar = nn.DataParallel(self.model.model_radar, device_ids=[0,1])
            
            avg_loss = []
            for idx_iter, (dict_datum, data2) in enumerate(tqdm(zip(data_loader_train, data_loader_train_2))):
                if self.optim_fastai:
                    self.scheduler.step(accumulated_iter, epoch)
                
                loss, tb_dict = self.model(dict_datum, data2) #KRKD forward results
                ...
                if self.optim_fastai:
                    ...
                    clip_grad_norm_(self.model.model_radar.parameters(), cfg_optim.GRAD_NORM_CLIP)
            ...
            if self.is_save_model:
                ...
                path_dict_model_1 = os.path.join(self.path_log, 'models_1', f'model_{epoch}.pt')
                path_dict_model_2 = os.path.join(self.path_log, 'models_2', f'model_{epoch}.pt')
                path_dict_util_1 = os.path.join(self.path_log, 'utils_1', f'util_{epoch}.pt')
                path_dict_util_2 = os.path.join(self.path_log, 'utils_2', f'util_{epoch}.pt')

                if (epoch+1) % self.interval_epoch_model == 0:
                    torch.save(self.model.model_radar1.state_dict(), path_dict_model_1)
                    torch.save(self.model.model_radar2.state_dict(), path_dict_model_2)
                if (epoch+1) % self.interval_epoch_util == 0:
                    dict_util_1 = {
                        'epoch': epoch,
                        'model_state_dict': self.model.model_radar1.state_dict(),
                        'optimizer_state_dict': self.optimizer.state_dict(),
                        'idx_log_iter': idx_log_iter, 
                    }
                    dict_util_2 = {
                        'epoch': epoch,
                        'model_state_dict': self.model.model_radar2.state_dict(),
                        'optimizer_state_dict': self.optimizer.state_dict(),
                        'idx_log_iter': idx_log_iter, 
                    }
                    if self.optim_fastai:
                        dict_util_1.update({'it': accumulated_iter})
                    else:
                        if not (self.scheduler is None):
                            dict_util_1.update({'scheduler_state_dict': self.scheduler.state_dict()})
                            dict_util_2.update({'scheduler_state_dict': self.scheduler.state_dict()})
                    torch.save(dict_util_1, path_dict_util_1)
                    torch.save(dict_util_2, path_dict_util_2)
            ...
            if self.is_validate:
                if self.is_consider_subset:
                    if ((epoch + 1) % self.val_per_epoch_subset) == 0:
                        print('#########################model_1############################')
                        self.validate_kitti_1(epoch, list_conf_thr=self.list_val_conf_thr, is_subset=True)
                        print('#########################model_2############################')
                        self.validate_kitti_2(epoch, list_conf_thr=self.list_val_conf_thr, is_subset=True)
                if ((epoch + 1) % self.val_per_epoch_full) == 0:
                    print('#########################model_1############################')
                    self.validate_kitti_1(epoch, list_conf_thr=self.list_val_conf_thr)
                    print('#########################model_2############################')
                    self.validate_kitti_2(epoch, list_conf_thr=self.list_val_conf_thr)

    def load_dict_model(self, models, path_dict_model, is_strict=False):
        pt_dict_model = torch.load(path_dict_model)
        models.load_state_dict(pt_dict_model, strict=is_strict)
        
        # 모델의 파라미터를 프리즈
        # for param in models.parameters():
        #     param.requires_grad = False
    
    def load_dict_model_lidar(self, path_dict_model, is_strict=False):
        pt_dict_model = torch.load(path_dict_model)
        self.model.model_lidar.load_state_dict(pt_dict_model, strict=is_strict)
    ...
    def vis_infer(self, sample_indices, conf_thr=0.7, is_nms=True, vis_mode=['lpc', 'spcube', 'cube'], is_train=False):
        ...
        * vis_mode (TBD)
        ...
        for dict_datum in data_loader:
            dict_out = self.model.model_radar(dict_datum)
            dict_out = self.model.model_radar.list_modules[-1].get_nms_pred_boxes_for_single_sample(dict_out, conf_thr, is_nms)
    ...
    def validate_kitti_conditional_1(self, epoch=None, list_conf_thr=None, is_subset=False, is_print_memory=False):
        self.model.model_radar1.eval()
        ...
        path_dir = os.path.join(self.path_log, 'test_kitti_1', dir_epoch)
        ...
        for idx_datum, dict_datum in enumerate(data_loader):
            ...
            try:
                dict_out = self.model.model_radar1(dict_datum) # inference
            ...
            for conf_thr in list_conf_thr:
                ...
                if is_feature_inferenced:
                    if eval_ver2:
                        ...
                        dict_out_current = self.model.model_radar1.list_modules[-1].get_nms_pred_boxes_for_single_sample(dict_out, conf_thr, is_nms=True)
                else:
    ...
    def validate_kitti_conditional_2(self, epoch=None, list_conf_thr=None, is_subset=False, is_print_memory=False):
        self.model.model_radar2.eval()
        ...
        # initialize via VAL.LIST_VAL_CONF_THR
        path_dir = os.path.join(self.path_log, 'test_kitti_2', dir_epoch)
        for conf_thr in list_conf_thr:
            os.makedirs(os.path.join(path_dir, f'{conf_thr}'), exist_ok=True)
        ...
        for idx_datum, dict_datum in enumerate(data_loader):
            ...
            try:
                dict_out = self.model.model_radar2(dict_datum) # inference
                is_feature_inferenced = True
            except:
                print('* Exception error (Pipeline): error during inferencing a sample -> empty prediction')
                print('* Meta info: ', dict_out['meta'])
            ...
            for conf_thr in list_conf_thr:
                ...
                if is_feature_inferenced:
                    if eval_ver2:
                        ...
                        dict_out_current = self.model.model_radar2.list_modules[-1].get_nms_pred_boxes_for_single_sample(dict_out, conf_thr, is_nms=True)
                else:
                    dict_out_current = update_dict_feat_not_inferenced(dict_out) # mostly sleet for lpc (e.g. no measurement)
    ...
    def validate_kitti_1(self, epoch=None, list_conf_thr=None, is_subset=False):
            self.model.model_radar1.training = False
            self.model.model_radar1.eval()
    ...
    def validate_kitti_2(self, epoch=None, list_conf_thr=None, is_subset=False):
            self.model.model_radar2.training=False
            self.model.model_radar2.eval()
