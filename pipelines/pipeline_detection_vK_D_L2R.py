...
from models.skeletons.KRKD import KRKD

class PipelineDetection_vK_D_L2R():
    def __init__(self, path_cfg=None, mode='train', path_cfg_teacher=None):
        '''
        * mode in ['train', 'test', 'vis']
        *   'train' denotes both train & test
        *   'test'  denotes mode for inference
        '''
        cfg_teacher = cfg.copy()
        self.cfg = cfg_from_yaml_file(path_cfg, cfg)
        self.cfg_teacher = cfg_from_yaml_file(path_cfg_teacher, cfg_teacher)
        self.mode = mode
        ...
        self.model = KRKD(model_cfg= self.cfg.KRKD_CFG)
        self.model.model_lidar = build_network_KD(self).cuda()
        self.model.model_radar = build_network(self).cuda()

        self.optimizer = build_optimizer(self, self.model.model_radar)
    ...
    def train_network(self, is_shuffle=True):
        ...
        self.load_dict_model_lidar(self.cfg_teacher['MODEL'].PRETRAINED)
        # self.model.model_radar.load_params_from_file()

        for p in self.model.model_lidar.parameters():
        ...
        for epoch in range(epoch_start, epoch_end):
            ...
            self.model.model_radar.train()
            self.model.model_radar.training = True
            ...
            for idx_iter, dict_datum in enumerate(tqdm(data_loader_train)):
                ...
                dict_datum = self.model.model_radar(dict_datum)
                loss, tb_dict = self.model(dict_datum) #KRKD forward results
            ...
            if self.is_save_model:
                ...
                if (epoch+1) % self.interval_epoch_model == 0:
                    torch.save(self.model.model_radar.state_dict(), path_dict_model)
                if (epoch+1) % self.interval_epoch_util == 0:
                    dict_util = {
                        'epoch': epoch,
                        'model_state_dict': self.model.model_radar.state_dict(),
    ...
    def load_dict_model(self, path_dict_model, is_strict=False):
        ...
        self.model.model_radar.load_state_dict(pt_dict_model, strict=is_strict)
    ...
    def validate_kitti(self, epoch=None, list_conf_thr=None, is_subset=False):
        self.model.model_radar.training=False
        self.model.model_radar.eval()
        ...
        for idx_datum, dict_datum in enumerate(data_loader):
            ...
            try:
                dict_out = self.model.model_radar(dict_datum) # inference
            ...
            for conf_thr in list_conf_thr:
                ...
                if is_feature_inferenced:
                    if eval_ver2:
                        ...
                        dict_out_current = self.model.model_radar.list_modules[-1].get_nms_pred_boxes_for_single_sample(dict_out, conf_thr, is_nms=True)
                else:
