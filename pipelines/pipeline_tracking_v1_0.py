...
from utils.box_utils import box3d_lidar_to_box2d, boxes_iou, boxes_giou, cal_iou_3d
...
class PipelineTracking_v1_0():
    def __init__(self, path_cfg=None, mode='train'):
        ...
        *   'test'  denotes mode for inference
        ...
        self.roi_size = self.model_cfg.LOSS_CONFIG.ROI_ALIGN_SIZE
        ...
        # Prediction
        self.prediction = build_prediction(self)
    ...
    def validate_kitti_conditional(self, epoch=None, list_conf_thr=None, is_subset=False, is_print_memory=False):
        ...
        for idx_datum, dict_datum in enumerate(data_loader):
            ...
            try:
                dict_datum['gt_boxes'] = dict_datum['gt_boxes'].cuda()
                dict_out = self.network(dict_datum) # inference
                is_feature_inferenced = True
            except:
                print('* Exception error (Pipeline): error during inferencing a sample -> empty prediction')
    ...
    def convert_3d_boxes_to_bev_format(self, boxes_3d):
        """
        Args:
            boxes_3d: (n, 8) shaped tensor, each row [batch_idx, x, y, z, dx, dy, dz, theta]

        Returns:
            rois_2d: (n, 5) shaped tensor, each row [batch_idx, x1, y1, x2, y2]
        """
        n = boxes_3d.shape[0]
        rois_2d = torch.zeros((n, 5), dtype=boxes_3d.dtype, device=boxes_3d.device)

        for i in range(n):
            x, y, _, dx, dy, _, theta = boxes_3d[i]

            # Calculate the corners based on rotation theta
            cos_theta = math.cos(theta)
            sin_theta = math.sin(theta)

            corner_x1 = x - dx/2 * cos_theta - dy/2 * sin_theta
            corner_y1 = y - dx/2 * sin_theta + dy/2 * cos_theta
            corner_x2 = x + dx/2 * cos_theta + dy/2 * sin_theta
            corner_y2 = y + dx/2 * sin_theta - dy/2 * cos_theta

            rois_2d[i] = torch.tensor([0, corner_x1, corner_y1, corner_x2, corner_y2], device=boxes_3d.device)

        return rois_2d

    def visualize(self, dict_item):
        ldr_path = dict_item['meta'][0]['path']['ldr64']
        pcd = o3d.io.read_point_cloud(ldr_path)
        ...
        plt.scatter(points[:, 0], points[:, 1], s=1)  # s is the size of points
        plt.xlabel('X')
        plt.ylabel('Y')
        ...
        gt_boxes_label = dict_item['meta'][0]['label']
        gt_id = []
        for gt in gt_boxes_label:
            gt_id.append(str(gt[2]))
        gt_boxes = box3d_lidar_to_box2d(dict_item['gt_boxes'][0][:,:-1].cpu())
            
        # gt 
        for idx, box in enumerate(gt_boxes):
            x1, y1, x2, y2 = box
            plt.plot([x1, x1, x2, x2, x1], [y1, y2, y2, y1, y1], color='black')

            #plt.text(x1, y1-1, gt_id[idx], color='black', fontsize=8)

        # kf_model prediction
        try:
            pred_boxes = box3d_lidar_to_box2d(torch.tensor(dict_item['output_box']))
            pred_id = dict_item['output_id']
            for idx, box in enumerate(pred_boxes):
                x1, y1, x2, y2 = box
                plt.plot([x1, x1, x2, x2, x1], [y1, y2, y2, y1, y1], color='red')

                plt.text(x1, y1-1, pred_id[idx], color='red', fontsize=10)                
        except:
            pass            
            
        # detection 

        det_boxes = box3d_lidar_to_box2d(torch.tensor(dict_item['pred_dicts'][0]['pred_boxes'].cpu()))
        for idx, box in enumerate(det_boxes):
            x1, y1, x2, y2 = box
            plt.plot([x1, x1, x2, x2, x1], [y1, y2, y2, y1, y1], color='blue')
    

        # #JDE detection
        # det_prediction = box3d_lidar_to_box2d(dict_item['list_alived_objs'].cpu().detach().numpy())
        # conf_score = dict_item['pred_dicts'][0]['pred_scores'].cpu().detach().numpy()
        # for idx, box in enumerate(det_prediction):
        #     if conf_score[idx] > 0.15:
        #         x1, y1, x2, y2 = box
        #         plt.plot([x1, x1, x2, x2, x1], [y1, y2, y2, y1, y1], color='green')

        #         #plt.text(x1, y1-2,conf_score[idx], color='green', fontsize=15)
    ...
    def tracking_inference(self):
        self.network.eval()

        data_loader = torch.utils.data.DataLoader(self.dataset_test, \
                batch_size = 1, shuffle = False, collate_fn = self.dataset_test.collate_fn, \
                num_workers = self.cfg.OPTIMIZER.NUM_WORKERS)
        ...
        for idx_datum, dict_datum in enumerate(data_loader):
            ...
            #         print(f'Seqeunce {prev_seq} Metric:')
            #         print(summary)
            ...
            dict_out = self.network(dict_datum)
            dict_out = self.prediction.forward(dict_out)
            
            ###### visualize ######
            # self.visualize(dict_out)

            #gt for evaluation
            gt_boxes_label = dict_out['meta'][0]['label']
            gt_id = []
            for gt in gt_boxes_label:
                gt_id.append(str(gt[2]))
            #gt_boxes = box3d_lidar_to_box2d(dict_out['gt_boxes'][0][:,:-1].cpu()) # 2D IoU
            gt_boxes = dict_out['gt_boxes'][0][:,:-1].cpu()

            #pred for evaluation
            try:
                #pred_boxes = box3d_lidar_to_box2d(torch.tensor(dict_out['output_box'])) # 2D IoU
                pred_boxes = torch.tensor(dict_out['output_box'])
                pred_id = dict_out['output_id']
            except:
                pass            

        ###### evaluate ######
            try:
                distances = np.zeros((len(gt_boxes), len(pred_boxes)), dtype=np.float32)
                for i, gt_box in enumerate(gt_boxes):
                    for j, pred_box in enumerate(pred_boxes):
                        distances[i, j] = 1 - cal_iou_3d(gt_box, pred_box)
                acc.update(gt_id, pred_id, distances)
                summary = mh.compute(acc, metrics=['mota', 'motp'], name='acc')
                print(f"Metrics after frame {idx_datum}:")
                print(summary)
            except:
                pass
    
        final_summary = mh.compute(acc, metrics=['mota', 'motp'], name='acc')
        print("Final Metrics:")
        print(final_summary)
