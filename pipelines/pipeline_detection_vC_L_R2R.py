...
from models.skeletons.radar_KRCLKD import radar_KRCLKD
from models.skeletons.radar_prog_KD import radar_prog_KD
...
class PipelineDetection_vC_L_R2R():
    def __init__(self, path_cfg_student=None, mode='train', path_cfg_1=None, path_cfg_2=None, path_cfg_3=None):
        '''
        ...
        *   'test'  denotes mode for inference
        ...
        self.cfg = cfg_from_yaml_file(path_cfg_student, cfg)
        
        self.cfg1 = cfg_from_yaml_file(path_cfg_1, cfg_1)
        self.cfg2 = cfg_from_yaml_file(path_cfg_2, cfg_2)
        self.cfg3 = cfg_from_yaml_file(path_cfg_3, cfg_3)
        
        self.cfg1 = AttrDict(self.cfg1)
        self.cfg2 = AttrDict(self.cfg2)
        self.cfg3 = AttrDict(self.cfg3)
        ...
        self.dataset_train = build_dataset(self, split='train') if self.mode == 'train' else None # student&teacher1
        self.dataset_train_1 = build_dataset_KD(self.cfg1, split='train') if self.mode == 'train' else None
        self.dataset_train_2 = build_dataset_KD(self.cfg2, split='train') if self.mode == 'train' else None
        self.dataset_train_3 = build_dataset_KD(self.cfg3, split='train') if self.mode == 'train' else None
        ...
        self.model = radar_KRCLKD(model_cfg= self.cfg.KRKD_CFG)
        # self.model = radar_prog_KD(model_cfg= self.cfg.KRKD_CFG)
        self.model.model_student = build_network(self).cuda()
        self.model.model_teacher1 = build_network_multi(self.cfg1).cuda() if self.mode == 'train' else None
        self.model.model_teacher2 = build_network_multi(self.cfg2).cuda() if self.mode == 'train' else None
        self.model.model_teacher3 = build_network_multi(self.cfg3).cuda() if self.mode == 'train' else None
        ...
        if self.cfg.GENERAL.LOGGING.IS_LOGGING:
            self.set_logging(path_cfg_student)
    ...
    def train_network(self, is_shuffle=True):
        ...
        #teacher1
        data_loader_train_1 = torch.utils.data.DataLoader(self.dataset_train_1, \
            batch_size = self.cfg.OPTIMIZER.BATCH_SIZE, sampler= common_sampler, \
            collate_fn = self.dataset_train_1.collate_fn,
            num_workers = 0, pin_memory=False,
            persistent_workers=False,
            drop_last = True)
        ...
        #teacher3
        data_loader_train_3 = torch.utils.data.DataLoader(self.dataset_train_3, \
            batch_size = self.cfg.OPTIMIZER.BATCH_SIZE, sampler= common_sampler, \
            collate_fn = self.dataset_train_3.collate_fn,
            num_workers = 0, pin_memory=False,
            persistent_workers=False,
            drop_last = True)

        # Pretrained Teacher Model with Freeze
        self.load_dict_model(self.model.model_teacher1, self.cfg1['MODEL'].PRETRAINED)
        self.load_dict_model(self.model.model_teacher2, self.cfg2['MODEL'].PRETRAINED)
        self.load_dict_model(self.model.model_teacher3, self.cfg3['MODEL'].PRETRAINED)
        ...
        for epoch in range(epoch_start, epoch_end):
            ...
            # Set Student Model train mode
            self.model.model_student.train()
            self.model.model_student.training = True
            
            avg_loss = []
            # for idx_iter, dict_datum in enumerate(tqdm(data_loader_train)):
            for idx_iter, (dict_datum, data1, data2, data3) in enumerate(tqdm(zip(data_loader_train,data_loader_train_1, data_loader_train_2, data_loader_train_3), total=len(data_loader_train), leave=False)):
                
                # seq = dict_datum['meta'][0]['seq']
                # if seq == '11':
                #     if dict_datum['meta'][0]['idx']['rdr'] == '00154':
                #         pass
                #     else:
                #         continue
                # else:
                ...
                loss, tb_dict = self.model(dict_datum, data1, data2, data3) #KRCLKD forward results
                ...
                if 'pointer' in data1.keys():
                    for dict_item in data1['pointer']:
                        for k in dict_item.keys():
                            if k != 'meta':
                                dict_item[k] = None
                for temp_key in data1.keys():
                    data1[temp_key] = None

                if 'pointer' in data2.keys():
                    for dict_item in data2['pointer']:
                        for k in dict_item.keys():
                            if k != 'meta':
                                dict_item[k] = None
                for temp_key in data2.keys():
                    data2[temp_key] = None

                if 'pointer' in data3.keys():
                    for dict_item in data3['pointer']:
                        for k in dict_item.keys():
                            if k != 'meta':
                                dict_item[k] = None
                for temp_key in data3.keys():
                    data3[temp_key] = None
            ...
            if self.is_save_model:
                ...
                if (epoch+1) % self.interval_epoch_model == 0:
                    torch.save(self.model.model_student.state_dict(), path_dict_model)
                if (epoch+1) % self.interval_epoch_util == 0:
                    dict_util = {
                        'epoch': epoch,
                        'model_state_dict': self.model.model_student.state_dict(),
    ...
    def validate_kitti(self, epoch=None, list_conf_thr=None, is_subset=False):
        self.model.model_student.training=False
        self.model.model_student.eval()
        ...
        for idx_datum, dict_datum in enumerate(data_loader):
            ...
            try:
                dict_out = self.model.model_student(dict_datum) # inference
            ...
            for conf_thr in list_conf_thr:
                ...
                if is_feature_inferenced:
                    if eval_ver2:
                        ...
                        dict_out_current = self.model.model_student.list_modules[-1].get_nms_pred_boxes_for_single_sample(dict_out, conf_thr, is_nms=True)
                else:
    ...
    def validate_kitti_conditional(self, epoch=None, list_conf_thr=None, is_subset=False, is_print_memory=False):
        ...
        for idx_datum, dict_datum in enumerate(data_loader):
            # if dict_datum['meta'][0]['seq'] == '11' and dict_datum['meta'][0]['idx']['rdr']=='00154':
            #     pass
            # else:
            #     continue
            ...
            if is_print_memory:
                ...
                print('max_memory: ', torch.cuda.max_memory_allocated(device='cuda')/1024/1024)
