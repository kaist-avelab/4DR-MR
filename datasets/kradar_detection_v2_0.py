...
class KRadarDetection_v2_0(Dataset):
    def __init__(self, cfg=None, split='all'):
        ...
        self.extra_process = self.rdr_sparse.get('extra_process', False)
    ...
    def get_heatmap(self, dict_item):
        heatmap = np.zeros((80,180))
        tuple_obj = dict_item['meta']['label']
        for obj in tuple_obj:
            _, vals, _, _ = obj
            x, y, _, _, l, w, _ = vals
            
            #transform to heatmap coordinate
            x = np.round(x*2.5)
            y = np.round(y*2.5) + 40
            
            half_size = 15
            start_x = int(max(x - half_size, 0))
            end_x = int(min(x + half_size, heatmap.shape[1]))
            start_y = int(max(y - half_size, 0))
            end_y = int(min(y + half_size, heatmap.shape[0]))

            heatmap[start_y:end_y, start_x:end_x] = 1

        dict_item['heatmap'] = heatmap
    ...
    def get_rdr_sparse(self, dict_item):
        if self.rdr_sparse.processed:
            ...
            if dir_rdr_sparse.split('/')[-1][:10] == 'rtnh_wider':
                path_rdr_sparse = osp.join(dir_rdr_sparse, seq, f'sprdr_{rdr_idx}.npy') 
            elif dir_rdr_sparse.split('/')[-1][0:8] == 'filtered':
                path_rdr_sparse = osp.join(dir_rdr_sparse, seq, f'rp_{rdr_idx}.npy') 
            elif dir_rdr_sparse.split('/')[-1][:5] == 'rtnhp':
                path_rdr_sparse = osp.join(dir_rdr_sparse, seq, f'rp_{rdr_idx}.npy') 
            else:
        ...
        if self.extra_process:
            log_pw = np.log1p(rdr_sparse[:, 3])
            rdr_sparse[:, 3] = (log_pw - np.min(log_pw)) / (np.max(log_pw) - np.min(log_pw) + 1e-6)
            rdr_sparse = rdr_sparse[:, (0,1,2,3)]
    ...
    def __getitem__(self, idx):
        ...
        dict_item = self.get_heatmap(dict_item) if self.item['heatmap'] else dict_item
        ...
        if self.collate_ver == 'v1_0':
            ...
            for batch_idx, dict_item in enumerate(list_batch):
                for k, v in dict_item.items():
                    ...
                    elif k in ['rdr_sparse', 'ldr64', 'heatmap']:
                        dict_batch[k].append(torch.from_numpy(dict_item[k]).float())
            dict_batch['batch_size'] = batch_idx+1
